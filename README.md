...
`board-tui --snapshot` prints the committed state as JSON; `GET /v1/snapshot` also works with `curl --unix-socket`. `board-tui --reconcile` requests a background refresh; success acknowledges scheduling, not completion. `make install` stops any running daemon so the just-installed binary takes effect; just reopen the board (it auto-spawns the new daemon). To restart manually, run `board-tui --daemon-stop`, then open the board again. See [the storage design](docs/design/host-state.md) for ownership and recovery.
