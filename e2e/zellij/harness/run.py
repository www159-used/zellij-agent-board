...
class PtyClient:
    ...
    def transcript(self) -> str:
        # The buffer only grows, so the decoded text is cached until the next
        # read. Pollers call this several times per tick.
        with self._lock:
...
class LiveWorld:
    def __init__(
        ...
        # Facts of the Experiment, fixed for the World's whole life.
        self.needs_board = _needs_board(experiment)
        self.explained_reconnect = _explains_reconnect(experiment)
        self.sentinels = frozenset(
            pane.sentinel
            for session in experiment.world.sessions
    ...
    def env(self) -> dict[str, str]:
        ...
        env["ZAB_JUMP_TRACE"] = str(self.state / "jump-trace")
    ...
    def _disturb_go(self, step: Disturb) -> str | None:
        if step.target is None or self.pty is None:
            return "world_never_quiet"
        tui = self._find_tui()
        if tui is None:
            return "permission_denied"
        if not self.pty.write(b"\r"):
            return "client_lost"
        self.notes.append(
    ...
    def bundle(self, verdict: str, hold: str | None = None, evidence: str | None = None) -> None:
        ...
        jump_trace = self.state / "jump-trace"
        if jump_trace.is_file():
            shutil.copy2(jump_trace, self.trial / "jump-trace")
        copied_logs = []
    ...
    def _write_config(self) -> None:
        ...
        if switch_session is not None:
            target = self.real_names[switch_session]
    ...
    def _open_board(self) -> str | None:
        ...
        while time.monotonic() < deadline:
            if "allow" in self.pty.transcript().lower():
                if not self.pty.write(b"y"):
                    return "client_lost"
            # The pane keeps its id for as long as this call is waiting, so
            # resolve it once and re-check only the rendered chrome.
            if tui is None:
                tui = self._find_tui()
    ...
    def _control_alive(self) -> bool:
        ...
        # is enough. The attach session is one of them, so this sweep is total.
        for real in self.real_names.values():
    ...
    def _panes(self, logical: str, *extra: str) -> list[dict]:
        return self._panes_in(self.real_names[logical], *extra)
    ...
    def _find_named_pane(self, real: str, role: str) -> int | None:
        for pane in self._panes_in(real, "--command"):
            blob = _pane_blob(pane, _NAMED_PANE_KEYS)
    ...
    def _find_tui(self) -> int | None:
        attach = self.real_names[self.experiment.world.attach]
        # After a jump the TUI may already be gone; look at the attach session
        # first. dict.fromkeys keeps that order and drops the duplicate entry.
        for real in dict.fromkeys((attach, *self.real_names.values())):
            for pane in self._panes_in(real, "--command"):
                if pane.get("is_plugin"):
                    continue
                if "board-tui" in _pane_blob(pane, _TUI_PANE_KEYS):
                    return int(pane["id"])
        return None
...
# Which `list-panes --json` fields carry the text a lookup matches against.
_NAMED_PANE_KEYS = ("title", "pane_command", "terminal_command", "name")
_TUI_PANE_KEYS = ("title", "pane_command", "terminal_command")


def _pane_blob(pane: dict, keys: tuple[str, ...]) -> str:
    return " ".join(str(pane.get(key) or "") for key in keys)
